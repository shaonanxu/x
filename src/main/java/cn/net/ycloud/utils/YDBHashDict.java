...
public class YDBHashDict {
	
	private final static int F = 7;
	private final static int CHARS_DICT_SIZE = 5;
	public static YDBHashDict initDict(String[] terms) {
		...
		YDBHashDict dict = new YDBHashDict(min, max);
		for(String a : terms) {
	...
	private YDBHashDict(int min, int max) {
		this.base = min - 1;
		int len = max - base + 1;
	...
	private class Terms {
		...
		int size;
		Terms(String term){
			this.terms = new char[CHARS_DICT_SIZE][];
		...
		private void ensureCapacity() {
			if(this.size == this.terms.length) {
				char[][] newTerms = new char[this.size + CHARS_DICT_SIZE][];
